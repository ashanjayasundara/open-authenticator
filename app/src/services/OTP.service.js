...
export default class OTPService {
  ...
  truncate(hmac) {
    ...
    otp = (this.hex2dec(hmac.toString().substr(offset * 2, 8)) & this.hex2dec("7fffffff")) + "";
  ...
  getTOTP(secret, now = new Date().getTime()) {
    // var hmac, key, otp, shaObj, time;

    // key = this.base32tohex(secret);
    // time = this.calculateTimeCounter(now);
    // var shaObj = new jsSHA("SHA-1", "HEX");
    // shaObj.setHMACKey(key, "HEX");
    // shaObj.update(time);
    // var hmac = shaObj.getHMAC("HEX");
    // otp = this.mangleHmac(hmac);
    // return otp;
